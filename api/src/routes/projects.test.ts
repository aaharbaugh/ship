...
describe('Projects API', () => {
  ...
  describe('GET /api/projects', () => {
    it('returns array with ice_score computed field', async () => {
      ...
      mockQuery.mockResolvedValueOnce(rowsResult(mockProjects));
  ...
  describe('POST /api/projects', () => {
    it('creates project without owner_id (optional)', async () => {
      ...
      mockQuery
        .mockResolvedValueOnce(rowsResult([mockProject]));
    ...
    it('creates project with valid data including optional owner_id', async () => {
      ...
      mockQuery
        ...
        .mockResolvedValueOnce(rowsResult([mockProject]))
        // Get user info
        .mockResolvedValueOnce(rowsResult([{ id: ownerId, name: 'Test Owner', email: 'owner@example.com' }]));
  ...
  describe('GET /api/projects/:id', () => {
    it('returns project with ice_score computed', async () => {
      ...
      mockQuery.mockResolvedValueOnce(rowsResult([mockProject]));
  ...
  describe('PATCH /api/projects/:id', () => {
    it('updates ICE properties', async () => {
      ...
      mockQuery
        // Check existing
        .mockResolvedValueOnce(rowsResult([existingProject]))
        // Update
        .mockResolvedValueOnce(rowsResult([]))
        // Re-query
        .mockResolvedValueOnce(rowsResult([updatedProject]));
  ...
  describe('DELETE /api/projects/:id', () => {
    it('deletes project and removes references', async () => {
      mockQuery
        // Access check
        .mockResolvedValueOnce(rowsResult([{ id: 'project-123' }]))
